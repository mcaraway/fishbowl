module Fishbowl::Requests
  def self.save_sales_order(options = {})
    puts "Start save_sales_order" if Fishbowl.configuration.debug.eql? true
    options = options.symbolize_keys
    
    puts "validating required fields" if Fishbowl.configuration.debug.eql? true
    %w{rows}.each do |required_field|
      raise ArgumentError if options[required_field.to_sym].nil?
    end    
    
    request = format_sales_order(options)
    puts "sending xml" if Fishbowl.configuration.debug.eql? true
...
private
  ...
  def self.format_sales_order(sales_order)
    # puts "formatting sales order xml with options #{sales_order}" if Fishbowl.configuration.debug.eql? true
    # puts "formatting sales order xml with options #{sales_order[:customer_name]}" if Fishbowl.configuration.debug.eql? true
    # puts "formatting sales order xml with options #{sales_order[:salesman]}" if Fishbowl.configuration.debug.eql? true
    # puts "formatting sales order xml with options #{sales_order[:status]}" if Fishbowl.configuration.debug.eql? true
    # puts "formatting sales order xml with options #{sales_order[:number]}" if Fishbowl.configuration.debug.eql? true
    # puts "formatting sales order xml with options #{sales_order[:payment_terms]}" if Fishbowl.configuration.debug.eql? true
    # puts "formatting sales order xml with options #{sales_order[:items]}" if Fishbowl.configuration.debug.eql? true
    Nokogiri::XML::Builder.new do |xml|
      xml.request {
        xml.ImportRq {
          xml.Type "ImportSalesOrder"
          xml.Rows {
            sales_order[:rows].each do |field|
              xml.Row field
            end
          }
          # xml.SalesOrder {
            ...
            # } unless sales_order[:bill_to].nil?
            # xml.Ship {
            ...
            # } unless sales_order[:ship_to].nil?
            # xml.CustomFields {
